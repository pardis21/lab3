  ...
  public boolean checkLogOrder() {
    ...
    for (int i = 1; i < logs.size(); i++) {
      ...
      switch (logs.get(i).method) {
        ...
        case ADD:
          for (int a = i; a >= 0; a++) {
            if (logs.get(a).value == logs.get(i).value) {
              switch (logs.get(a).method) {
                case CONTAINS:
                  if (logs.get(a).success == logs.get(i).success) {
                    System.out.println("add-contains " + i + " " + logs.get(i).timestamp);
                    return false;
                  }
                  break;
                case REMOVE:
                  if (!logs.get(i).success && logs.get(a).success) {
                    System.out.println("add-remove " + i + " " + logs.get(i).timestamp);
                    return false;
                  }
                  break;
                case ADD:
            ...
            if (a == 0 && !logs.get(i).success) {
              System.out.println("successful adding of item never added " + i + " " + logs.get(i).timestamp);
              return false;
            }
          }
      }
